...
install_test_apk() {
	...
	$ADB install -r ${OUTPUTS_DIR}/apk/androidTest/debug/habits-android-debug-androidTest.apk || fail
}
...
run_jvm_tests() {
	...
	if [ ! -z $RELEASE ]; then
		$GRADLE testReleaseUnitTest :habits-core:check || fail
	else
		$GRADLE testDebugUnitTest :habits-core:check || fail
	fi
}
